...
found:
  ...
  //allocate a tms struct
  struct tms *timer;
  if ((timer = (struct tms *)kalloc()) == 0)
  {
    release(&p->lock);
    return 0;
  }
...
freeproc(struct proc *p)
{
  ...
  p->trapframe = 0;
  if (p->pagetable)
    proc_freepagetable(p->pagetable, p->heap_addr);
  p->pagetable = 0;
  p->heap_addr = 0;
  p->pid = 0;
  p->parent = 0;
...
int fork(void)
{
  ...
  if (uvmcopy(p->pagetable, np->pagetable, p->heap_addr) < 0)
  {
    freeproc(np);
    release(&np->lock);
    return -1;
  }
  np->heap_addr = p->heap_addr;
...
int wait(int target_proc, uint64 addr)
{
  struct proc *np;
  int havekids, pid;
  struct proc *p = myproc();
...
struct mappedfile *
allocVma() {
    struct mappedfile *area;
    for (area = map; area < map + NVMA; area++) {
        if (area->f == 0) return area;
    }
    return 0;
}