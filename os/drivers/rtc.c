...
typedef struct _rtc_instance_t
{
    ...
    void *alarm_ctx;
    uint8 tick_is_single_shot;
    uint8 alarm_is_single_shot;
    uint8 tick_enable_by_alarm;
    uint8 tick_enable_by_user;
...
int rtc_alarm_set_mask(rtc_mask_t mask)
{
    ...
    interrupt_ctrl.alarm_compare_mask = *(uint8 *)&mask;
    /* Set RTC mode to timer setting mode */
    rtc_timer_set_mode(RTC_TIMER_SETTING);
    rtc->interrupt_ctrl = interrupt_ctrl;
    /* Set RTC mode to timer running mode */
...
rtc_mask_t rtc_alarm_get_mask(void)
{
    ...
    uint8 compare_mask = interrupt_ctrl.alarm_compare_mask;
...
int rtc_protect_set(int enable)
{
    ...
    if (enable) {
        ...
        register_ctrl.timer_mask = *(uint8 *)&unmask;
        register_ctrl.alarm_mask = *(uint8 *)&unmask;
        ...
        register_ctrl.timer_mask = *(uint8 *)&mask;
        register_ctrl.alarm_mask = *(uint8 *)&mask;
...
int rtc_irq_callback(void *ctx)
{
    ...
    if(rtc_alarm_get_interrupt())
    {
        ...
        if(instance->alarm_is_single_shot)
        {
            ...
            instance->tick_enable_by_alarm = 0;
...
tick:
    if(instance->tick_enable_by_user)
    {
        ...
        {
            rtc_tick_set_interrupt(0);
            instance->tick_enable_by_user = 0;
...
int rtc_tick_irq_register(uint8 is_single_shot, rtc_tick_interrupt_mode_t mode, plic_irq_callback_t callback, void *ctx, uint8 priority)
{
    ...
    rtc_instance.tick_enable_by_user = 1;
...
void rtc_tick_irq_unregister(void)
{
    ...
    rtc_instance.tick_enable_by_user = 0;
...
int rtc_alarm_irq_register(uint8 is_single_shot, rtc_mask_t mask, plic_irq_callback_t callback, void *ctx, uint8 priority)
{
    //plic_irq_disable(IRQN_RTC_INTERRUPT);
    ...
    rtc_instance.tick_enable_by_alarm = 1;
...
void rtc_alarm_irq_unregister(void)
{
    ...
    rtc_instance.tick_enable_by_alarm = 0;
