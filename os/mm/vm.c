...
extern int free_physical_pages;
...
uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
  ...
  for(a = oldsz; a < newsz; a += PGSIZE){
    ...
    if(mem == 0){
      ...
      out_point = 1;
      __debug_error("uvmalloc", "no free user memory\n");
      panic("uvmalloc");
      break;
    }
    memset(mem, 0, PGSIZE);
    if(mappages(pagetable, a, PGSIZE, (uint64)mem, PTE_W|PTE_X|PTE_R|PTE_U) != 0){
      kfree(mem);
      uvmdealloc(pagetable, a, oldsz);
      __debug_error("uvmalloc", "map page error\n");
      panic("uvmalloc");
      out_point = 2;
    }
  }
...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if((mem = kalloc()) == 0)
    {
      __debug_error("uvmcopy", "error to allocate memory!\n");
      goto err;
    }
    memmove(mem, (char*)pa, PGSIZE);
    if(mappages(new, i, PGSIZE, (uint64)mem, flags) != 0){
      kfree(mem);
      __debug_error("uvmcopy", "error to map page!\n");
      goto err;
    }
  }
  return 0;
